...
public class GooseHex : MonoBehaviour
{
    public GameObject TaskTextPrefab;
    private GameObject text;
    public int prefabLimit = 0;

    public GameObject HexButtonGoose;
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 80f && !GameObject.Find("waterGoose"))
        {
            HexButtonGoose.SetActive(true);
        }
        else HexButtonGoose.SetActive(false);
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("waterGoose") && Camera.main.fieldOfView <= 80f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
                text = Instantiate(TaskTextPrefab, GameObject.Find("HexBuildGoose").transform.position + new Vector3(0f, 1f, 0f), Quaternion.identity);
                prefabLimit++;
            }
        }
        else
    ...
    private void OnGUI()
    {
        ...
        var textSize = GUI.skin.label.CalcSize(new GUIContent("GooseHex"));

        guiStyle.fontSize = 75;

        if (!GameObject.Find("Background") && !GameObject.Find("waterGoose"))
        {
