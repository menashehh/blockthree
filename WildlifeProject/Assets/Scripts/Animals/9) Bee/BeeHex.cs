...
public class BeeHex : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 65f && !GameObject.Find("Tree Bee"))
        {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Tree Bee") && Camera.main.fieldOfView <= 65f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
