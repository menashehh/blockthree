...
public class WolfHex2 : MonoBehaviour
{
    public GameObject TaskTextPrefab;
    private GameObject text;
    public int prefabLimit = 0;

    public GameObject HexButtonWolf2;
    ...
    private void Update()
    {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Tree Wolf") && Camera.main.fieldOfView <= 80f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
                text = Instantiate(TaskTextPrefab, GameObject.Find("HexBuildWolf2").transform.position + new Vector3(0f, 1f, 0f), Quaternion.identity);
                prefabLimit++;
            }
        }
        else
    ...
    private void OnGUI()
    {
        ...
        var textSize = GUI.skin.label.CalcSize(new GUIContent("WolfHex2"));
    ...
    */
    ...
    private void OnMouseUp()
    {
        if (Camera.main.fieldOfView <= 80f && !GameObject.Find("Background") && !EventSystem.current.IsPointerOverGameObject()
            ...
            && !GameObject.Find("wolfText") && !GameObject.Find("Hex Button Wolf")
