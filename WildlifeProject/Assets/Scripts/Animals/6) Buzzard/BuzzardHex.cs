...
public class BuzzardHex : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 65f && !GameObject.Find("Buzzard Req"))
        {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Buzzard Req") && Camera.main.fieldOfView <= 65f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
                text = Instantiate(TaskTextPrefab, GameObject.Find("HexBuildBuzzard").transform.position + new Vector3(0f, 1f, 0f), Quaternion.identity);
                prefabLimit++;
            }
        }
        else
    ...
    */
    ...
    private void OnMouseUp()
    {
        if (Camera.main.fieldOfView <= 65f && !GameObject.Find("Background") && !EventSystem.current.IsPointerOverGameObject()
            ...
            && !GameObject.Find("buzzardText") && !GameObject.Find("Hex Button Buzzard 2")
