...
public class WormHex : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 65f && !GameObject.Find("Apple"))
        {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Apple") && Camera.main.fieldOfView <= 65f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
