...
public class WormHex4 : MonoBehaviour
{
    ...
    public GameObject HexButtonWorm4;
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 65f && !GameObject.Find("Grass Worm 2"))
        {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Grass Worm 2") && Camera.main.fieldOfView <= 65f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
                text = Instantiate(TaskTextPrefab, GameObject.Find("HexBuildWorm4").transform.position + new Vector3(0f, 1f, 0f), Quaternion.identity);
                prefabLimit++;
            }
        }
        else
    ...
    private void OnGUI()
    {
        ...
        var textSize = GUI.skin.label.CalcSize(new GUIContent("WormHex4"));

        guiStyle.fontSize = 75;

        if (!GameObject.Find("Background") && !GameObject.Find("Grass Worm 2"))
        {
