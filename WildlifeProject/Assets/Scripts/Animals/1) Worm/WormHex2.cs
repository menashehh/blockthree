...
public class WormHex2 : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 80f && !GameObject.Find("waterWorm"))
        {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("waterWorm") && Camera.main.fieldOfView <= 80f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
