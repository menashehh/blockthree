...
public class WormHex3 : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 80f && !GameObject.Find("Tree Worm"))
        {
            HexButtonWorm3.SetActive(true);
        }
        else HexButtonWorm3.SetActive(false);

        if (!GameObject.Find("Background") && !GameObject.Find("Tree Worm") && Camera.main.fieldOfView <= 80f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
