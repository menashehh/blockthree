...
public class SparrowHex : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 80f && !GameObject.Find("waterSparrow"))
        {
            HexButtonSparrow.SetActive(true);
        }
        else HexButtonSparrow.SetActive(false);

        if (!GameObject.Find("Background") && !GameObject.Find("waterSparrow") && Camera.main.fieldOfView <= 80f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
