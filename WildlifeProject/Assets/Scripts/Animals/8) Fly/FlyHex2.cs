...
public class FlyHex2 : MonoBehaviour
{
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 65f && !GameObject.Find("Grass Fly"))
        {
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Grass Fly") && Camera.main.fieldOfView <= 65f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
