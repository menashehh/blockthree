...
public class FlyHex : MonoBehaviour
{
    public GameObject TaskTextPrefab;
    private GameObject text;
    public int prefabLimit = 0;

    public GameObject HexButtonFly;
    ...
    private void Update()
    {
        if (menuOpen == 1 && Camera.main.fieldOfView <= 80f && !GameObject.Find("Fly Req"))
        {
            HexButtonFly.SetActive(true);
        }
        else HexButtonFly.SetActive(false);
        ...
        if (!GameObject.Find("Background") && !GameObject.Find("Fly Req") && Camera.main.fieldOfView <= 80f && menuOpen == 0)
        {
            if (prefabLimit != 1)
            {
                text = Instantiate(TaskTextPrefab, GameObject.Find("HexBuildFly").transform.position + new Vector3(0f, 1f, 0f), Quaternion.identity);
                prefabLimit++;
            }
        }
        else
    ...
    private void OnGUI()
    {
        ...
        var textSize = GUI.skin.label.CalcSize(new GUIContent("FlyHex"));

        guiStyle.fontSize = 75;

        if (!GameObject.Find("Background") && !GameObject.Find("Fly Req"))
        {
    ...
    */
    ...
    private void OnMouseUp()
    {
        if (Camera.main.fieldOfView <= 80f && !GameObject.Find("Background") && !EventSystem.current.IsPointerOverGameObject()
            ...
            && !GameObject.Find("flyText") && !GameObject.Find("Hex Button Fly 2")
