    ...
    */
    ...
    private void Update()
    {
       ...
       EventSystem.current.currentSelectedGameObject.CompareTag("Close Buttons"))
        ...
        if (!GameObject.Find("Background") && frogLevel.text != "Level 5" && Camera.main.fieldOfView <= 80f && npcName == false)
        {
            if (prefabLimit != 1)
            {
                text = Instantiate(TaskTextPrefab, GameObject.Find("Frog").transform.position + new Vector3(0f, 1f, 0f), Quaternion.identity);
                prefabLimit++;
            }
        }
        else
